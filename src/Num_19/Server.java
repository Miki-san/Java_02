...
public class Server {
    public static int receivePort = 9888;
    ...
    public static void executeCommand(String command, InetAddress ip){
        try {
            switch (command) {
                ...
                case "leave":
                    for (InetAddress  i : users.keySet()) {
                        sendMessage("User '" + users.get(ip) + "' left the chat.", i.toString(), sendPort);
                    }
                    users.remove(ip);
                    System.out.println("User '" + ip.toString() + "' | '" + users.get(ip) + "' left the chat.");
                    break;
                case "checkUser":
    ...
    public static void main(String[] args) throws UnknownHostException {
        ...
        System.out.println("Listening for " + receivePort);
        while (true) {
            ...
            if (data.startsWith("//")) {
                ...
                for (InetAddress add : users.keySet()) {
                    if (!add.toString().equals(packet.getAddress().toString())) {
                        try {
                            sendMessage(users.get(packet.getAddress()) + ": " + data, add.toString(), sendPort);
                        } catch (IOException exception) {
                            exception.printStackTrace();
                        }
                    }
